...
def main():
    ...
    # We have exactly 2 clusters. Let's look at their pitch (index 1 of the means)
    pitch_cluster_0 = gmm.means_[0][1]
    pitch_cluster_1 = gmm.means_[1][1]

    # The "On-Road" cluster is the one where the driver is looking higher up.
    # Note: In standard L2CS-Net, looking UP usually corresponds to a higher (more positive) pitch.
    # If your specific model maps looking DOWN as a positive number, simply change > to <.
    if pitch_cluster_0 > pitch_cluster_1:
        on_road_cluster_idx = 0
        phone_cluster_idx = 1
    else:
        on_road_cluster_idx = 1
        phone_cluster_idx = 0

    print(f"\n[GMM Info] Road Pitch: {gmm.means_[on_road_cluster_idx][1]:.1f}° | Phone Pitch: {gmm.means_[phone_cluster_idx][1]:.1f}°")
